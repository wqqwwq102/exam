...
<body>
	<div>
		<form action="searchClass.do" name="search" method="get">
			班级名:<input type="text" name="clsname"> 班级类型:<select
				name="typeId">
				<c:forEach items="${alist}" var="cl">
					<option value="${cl.type.typeId}">${cl.type.typeName}</option>
				</c:forEach>
			</select> 讲师:<input type="text" name="tname"> 班主任:<input name="hname"
				type="text"> 开班时间：<input type="text" name="strartdate"
				onfocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd',readOnly:true})">
			<select name="status">
				<option value=0>未开始</option>
				<option value=1>学习中</option>
				<option value=2>已结课</option>
			</select> <input type="submit" value="搜索"> <input id="addclass"
		...
		<table>
			...
			<c:forEach items="${clist}" var="cl">
				<tr>
					...
					<td><fmt:formatDate value="${cl.strartdate}"
	...
	<script type="text/javascript">
		...
		$("#x").delegate("select", "change", function() {
			...
			$(".do").each(function() {
				...
				if (selectId == "change") {
					var clsid = a.attr("id");
					location = "toUpdateClasses.do?clsid=" + clsid
				} else if (selectId == "study") {
					var clsid = a.attr("id");
					location = "studying.do?clsid=" + clsid
				} else if (selectId == "over") {
					var clsid = a.attr("id");
					location = "overClass.do?clsid=" + clsid
				}
			});
		});
	</script>
