...
public class ExamPaperController {
	...
	public ModelAndView listExamPaper() {
		...
		return new ModelAndView("/jsps/examination/examination_paper.jsp")
				.addObject("plist", plist).addObject("tlist", tlist);
	}
	...
	public ModelAndView toAddExamPaper() {
		List<Type> tlist = expaperService.listTypename();
		return new ModelAndView("/jsps/examination/add_paper.jsp").addObject(
				"tlist", tlist);
	}
	...
	public ModelAndView listThisExamPaper(int paperid) {
		...
		List<Type> tlist = expaperService.listTypename();
		return new ModelAndView("/jsps/examination/update_examPaper.jsp")
				.addObject("exam", exam).addObject("tlist", tlist);
	}
	...
	public ModelAndView showExamPaper(int paperid) {
		...
		for (ExaminationItem examinationitem : list) {
			ExaminationSelect a = JSON.parseObject(examinationitem.getOption(),
					ExaminationSelect.class);
			examinationitem.setSelects(a);
		}
		exam.setExamitem(list);
		return new ModelAndView("/jsps/examination/show_examPaper.jsp")
				.addObject("exam", exam);
	}

	@RequestMapping("listStuExamItem")
	public ModelAndView listStuExamItem(int eid) {
		Examinationpaper exam = expaperService.listStuExamItem(eid);
		List<ExaminationItem> list = expaperService.showExamPaper(exam);
